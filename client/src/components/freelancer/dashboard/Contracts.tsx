...
const Contracts = () => {
  ...
  const filterContracts = () => {
    let filtered = [...contracts]; // Create a copy of contracts
    ...
    if (searchQuery) {
      filtered = filtered.filter(
        (c) =>
          ...
          c.clientId.fullName.toLowerCase().includes(searchQuery.toLowerCase())
      );
    }

    // Apply status filter
    if (statusFilter !== 'all') {
      filtered = filtered.filter((c) => c.contractStatus === statusFilter);
    }
